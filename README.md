...
Test scripts name their files explicitly rather than globbing. `node --test 'test/*.test.ts'` exits **0** when the glob matches nothing, so a package whose test directory goes missing reports "0 tests, 0 failures" and CI stays green over it — which is exactly how `apps/server` came to have a test script and no tests. An explicit path fails loudly instead.
